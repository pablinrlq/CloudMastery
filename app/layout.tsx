    ...
    >
      <head>
        ...
        <Script id="cloudmastery-theme" nonce={nonce} strategy="beforeInteractive">
          {`(function(){try{var p=window.location.pathname;var app=/^(\\/dashboard|\\/course|\\/simulado|\\/flashcards|\\/certificado)(\\/|$)/.test(p);var t=app?'dark':localStorage.getItem('cm-theme');if(t==='dark'||(!t&&window.matchMedia('(prefers-color-scheme: dark)').matches)){document.documentElement.classList.add('dark')}else{document.documentElement.classList.remove('dark')}}catch(e){}})();`}
        </Script>
      </head>
