...
import { getModules, type CertId } from "@/lib/learning/content";
...
export async function getReadiness(certId: CertId): Promise<Readiness> {
  ...
  const attempts = await db.selectFrom("simulado_attempts")
    .select(["score", "domain_breakdown", "completed_at"])
    .where("user_id", "=", userId).where("cert_id", "=", certId).where("mode", "=", "full")
    .where("completed_at", "is not", null).orderBy("completed_at", "desc").limit(10).execute();
  ...
  const scoreHistory = [...completedAttempts]
    ...
    .map((a) => ({
      ...
      completedAt: new Date(a.completed_at!).toISOString(),
    }))
