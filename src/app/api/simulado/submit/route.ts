...
export async function POST(request: NextRequest) {
  ...
  const attempt = await db.selectFrom("simulado_attempts").select(["id", "user_id", "cert_id", "mode", "domain", "completed_at", "hints_used", "selected_question_ids"]).where("id", "=", attemptId).executeTakeFirst();
  ...
  const questions = await db.selectFrom("questions").select(["id", "domain", "prompt", "choices", "correct_choice_ids", "explanation"]).where("id", "in", questionIds).where("cert_id", "=", attempt.cert_id).execute();
  ...
  const completed = await db.updateTable("simulado_attempts").set({
      score,
    ...
    }).where("id", "=", attemptId).where("completed_at", "is", null).returning("id").executeTakeFirst();
  if (!completed) {
    return NextResponse.json({ error: "Tentativa já finalizada" }, { status: 409 });
  }
