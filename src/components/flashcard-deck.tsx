...
export function FlashcardDeck({ cards }: { cards: Flashcard[] }) {
  ...
  if (index >= ordered.length) {
    ...
    return (
      <div className="study-card p-8 text-center sm:p-12">
  ...
  return (
    <div>
      <div className="study-card mb-5 flex items-center justify-between p-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
      ...
      <div className="mb-6 h-1.5 overflow-hidden rounded-full bg-slate-200/70 dark:bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-orange-500 to-amber-300 transition-all duration-500" style={{ width: `${((index + 1) / ordered.length) * 100}%` }} /></div>

      <button
        onClick={() => setFlipped((f) => !f)}
        className="group relative flex min-h-[350px] w-full flex-col items-center justify-center overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white p-8 text-center shadow-[0_22px_60px_-38px_rgba(15,23,42,0.35)] transition-all duration-300 hover:-translate-y-1 hover:border-orange-200 hover:shadow-[0_30px_70px_-38px_rgba(15,23,42,0.45)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-500/15 dark:border-white/10 dark:bg-slate-900 dark:hover:border-orange-500/30 sm:p-12"
      >
        <span className="absolute left-0 top-0 h-1 w-full bg-gradient-to-r from-orange-500 via-amber-300 to-orange-500" />
        {card.domain && (
        ...
        <span className="mt-7 inline-flex items-center gap-2 text-xs font-semibold text-slate-400 transition group-hover:text-orange-500 dark:text-slate-500">
          <SparkIcon className="h-3.5 w-3.5" />{flipped ? "clique para ver a frente" : "clique para revelar"}
        </span>
      </button>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <button
          ...
          disabled={!flipped}
          className="inline-flex min-h-12 items-center justify-center rounded-xl border border-amber-200 bg-amber-50 px-4 text-sm font-bold text-amber-800 transition-all duration-200 hover:-translate-y-0.5 hover:border-amber-300 hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-500/15 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300"
        >
          <ClockIcon className="mr-2 h-4 w-4" />Revisar depois
        </button>
        <button
          onClick={() => mark("known")}
          disabled={!flipped}
          className="inline-flex min-h-12 items-center justify-center rounded-xl border border-emerald-200 bg-emerald-50 px-4 text-sm font-bold text-emerald-800 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-500/15 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300"
        >
          <CheckIcon className="mr-2 h-4 w-4" />Sei essa!
        </button>
      </div>
    </div>
  );
